    ...
    private void showMessageGameOver(Graphics g) {
        ...
        switch (stateGameOver) {
            ...
            case STATE_WIN_HUMAN:
                g.drawString("HUMAN Wins!", 100, getHeight() / 2 + 30);
                break;
            case STATE_WIN_AI:
                g.drawString("AI Wins!", 180, getHeight() / 2 + 30);
                break;
        }
    }
    ...
    private boolean checkWin(int c) {
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 1; x < fieldSizeX; x++) {
                if (!isCellEmpty(x, y) && (field[y][x] == field[y][0]))
                    countDot++;
            }
            if(countDot == fieldSizeX) return true;
            countDot = 1;
        }

        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 1; x < fieldSizeX; x++) {
                if (!isCellEmpty(x, y) && (field[x][y] == field[0][y]))
                    countDot++;
            }
            if(countDot == fieldSizeX) return true;
            countDot = 1;
        ...
        for (int y = 1; y < fieldSizeY; y++) {
            if (!isCellEmpty(y, y) && (field[y][y] == field[0][0]))
                countDot++;
        }
        if (countDot == fieldSizeX) return true;
        countDot = 1;
        for (int y = 1; y < fieldSizeY; y++) {
            if (!isCellEmpty(y, y) && (field[fieldSizeY - y - 1][y] == field[fieldSizeY - 1][0]))
                countDot++;
        }
        if (countDot == fieldSizeX) return true;
